...
import gaknn.core.kdtree.KDTreeNode;
import gaknn.core.kdtree.KDTreeNodeSplitter;
import gaknn.core.kdtree.SlidingMidPointOfWidestSide;
...
public class KDTree
  ...
  protected void buildKDTree(Instances instances) throws Exception {
    ...
    m_MaxDepth = 0;
    m_Root = new KDTreeNode(m_NumNodes, 0, m_Instances.Size() - 1,
        universe);
      ...
      if (heap.size() < k) { // if haven't found the first k
        double distanceToSplitPlane = distanceToParents
            + m_EuclideanDistance.sqDifference(node.m_SplitDim, target
                .GetElementAt(node.m_SplitDim), node.m_SplitValue);
  ...
  protected void addInstanceToTree(Instance inst, KDTreeNode node)
      throws Exception {
    if (node.isALeaf()) {
      int instList[] = new int[m_Instances.Size()];
      try {
        ...
        instList[node.m_End + 1] = m_Instances.Size() - 1;
  ...
  protected void checkMissing(Instances instances) throws Exception {
    for (int i = 0; i < instances.Size(); i++) {
      ...
      for (int j = 0; j < ins.numValues(); j++) {
        if (ins.index(j) != ins.GetClassIndex())
          if (ins.isMissing(j)) {
  ...
  protected int widestDim(double[][] nodeRanges, double[][] universe) {
    final int classIdx = m_Instances.GetClassIndex();
    double widest = 0.0;
    int w = -1;
  ...
  public void centerInstances(Instances centers, int[] assignments, double pc)
    ...
    int[] centList = new int[centers.Size()];
  ...
  protected boolean clipToInsideHrect(KDTreeNode node, Instance x) {
    ...
    for (int i = 0; i < m_Instances.NumAttributes(); i++) {
      ...
      if (x.GetElementAt(i) < node.m_NodeRanges[i][MIN]) {
        x.setValue(i, node.m_NodeRanges[i][MIN]);
        inside = false;
      } else if (x.GetElementAt(i) > node.m_NodeRanges[i][MAX]) {
